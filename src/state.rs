...
use cosmwasm_std::{Addr, ReadonlyStorage, StdError, StdResult, Storage};
...
pub const KEY_REBASE_COUNT: &[u8] = b"rebase-count";
...
impl<'a, S: Storage> Config<'a, S> {
    ...
    pub fn set_rebase_count(&mut self, count: u64) -> StdResult<()> {
        set_bin_data(&mut self.storage, KEY_REBASE_COUNT, &count)
    }
}
...
impl<'a, S: ReadonlyStorage> ReadonlyConfigImpl<'a, S> {
    ...
    pub fn rebase_count(&self) -> u64 {
        get_bin_data(self.0, KEY_REBASE_COUNT).unwrap_or_default()
    }
}

// GonBalances

pub struct ReadonlyGonBalances<'a, S: ReadonlyStorage> {
    storage: ReadonlyPrefixedStorage<'a>,
}

impl<'a, S: ReadonlyStorage> ReadonlyGonBalances<'a, S> {
    pub fn from_storage(storage: &'a S) -> Self {
        Self {
    ...
    fn as_readonly(&self) -> ReadonlyGonBalancesImpl<ReadonlyPrefixedStorage<S>> {
        ReadonlyGonBalancesImpl(&self.storage)
    }

    pub fn account_amount(&self, account: &Addr) -> U256 {
        self.as_readonly().account_amount(account)
    }
}

pub struct GonBalances<'a, S: Storage> {
    storage: PrefixedStorage<'a>,
}

impl<'a, S: Storage> GonBalances<'a, S> {
    pub fn from_storage(storage: &'a mut S) -> Self {
        Self {
    ...
    fn as_readonly(&self) -> ReadonlyGonBalancesImpl<PrefixedStorage<S>> {
        ReadonlyGonBalancesImpl(&self.storage)
    }

    pub fn gon_balance(&self, account: &Addr) -> U256 {
        self.as_readonly().account_amount(account)
    }

    pub fn set_account_gon_balance(&mut self, account: &Addr, amount: U256) {
        let mut big_endian : [u8;32] = [0; 32];
        amount.to_big_endian(&mut big_endian);
        self.storage.set(account.as_slice(), &big_endian)
    }
}

/// This struct refactors out the readonly methods that we need for `GonBalances` and `ReadonlyGonBalances`
/// in a way that is generic over their mutability.
///
/// This was the only way to prevent code duplication of these methods because of the way
/// that `ReadonlyPrefixedStorage` and `PrefixedStorage` are implemented in `cosmwasm-std`
struct ReadonlyGonBalancesImpl<'a, S: ReadonlyStorage>(&'a S);

impl<'a, S: ReadonlyStorage> ReadonlyGonBalancesImpl<'a, S> {
    pub fn account_amount(&self, account: &Addr) -> U256 {
        let account_bytes = account.as_slice();
        let result = self.0.get(account_bytes);
        match result {
            // This unwrap is ok because we know we stored things correctly
            Some(gon_balance_bytes) => slice_to_u256(&gon_balance_bytes).unwrap(),
            None => U256::zero(),
        }
    }
}
...
pub fn read_allowance<S: Storage>(
    ...
    let owner_store =
        ReadonlyPrefixedStorage::multilevel(&[PREFIX_ALLOWANCES, owner.as_str().as_bytes()], store);
    let allowance = owner_store.may_load(spender.as_str().as_bytes());
...
pub fn write_allowance<S: Storage>(
    ...
    let mut owner_store =
        PrefixedStorage::multilevel(&[PREFIX_ALLOWANCES, owner.as_str().as_bytes()], store);

    owner_store.store(spender.as_str().as_bytes(), &allowance)
}

// Viewing Keys

pub fn write_viewing_key<S: Storage>(store: &mut S, owner: &Addr, key: &ViewingKey) {
    let mut balance_store = PrefixedStorage::new(PREFIX_VIEW_KEY, store);
    balance_store.set(owner.as_slice(), &key.to_hashed());
}

pub fn read_viewing_key<S: Storage>(store: &S, owner: &Addr) -> Option<Vec<u8>> {
...
/// Converts 16 bytes value into u128
/// Errors if data found that is not 16 bytes
fn slice_to_u256(data: &[u8]) -> StdResult<U256> {
    match <[u8; 32]>::try_from(data) {
        Ok(bytes) => Ok(U256::from_big_endian(&bytes)),
        Err(_) => Err(StdError::generic_err(
            "Corrupted data found. 32 byte expected.",
        )),
    }
}
